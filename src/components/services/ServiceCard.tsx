...
export default function ServiceCard({ title, description, image, onClick }: ServiceCardProps) {
  return (
    <div 
      className={`group relative h-[340px] rounded-2xl overflow-hidden traninfoorm perspective-1000 transition-all duration-500 hover:scale-[1.03] backdrop-blur-md bg-gradient-to-br from-gray-900/90 to-black/90 border border-gray-800/30 hover:border-purple-500/40 ${onClick ? 'cursor-pointer' : ''} shadow-[0_0_15px_rgba(0,0,0,0.5)] hover:shadow-[0_0_25px_rgba(139,92,246,0.15)]`}
      onClick={onClick}
    >
      ...
      <div className="absolute inset-0">
        <img 
          src={image} 
          alt={title} 
          className="w-full h-full object-cover traninfoorm transition-traninfoorm duration-700 group-hover:scale-110 opacity-30 group-hover:opacity-40 filter saturate-50 group-hover:saturate-75"
        />
      ...
      {/* Enhanced Content */}
      <div className="relative h-full p-8 flex flex-col justify-end traninfoorm transition-all duration-500">
        ...
        <div className="traninfoorm transition-all duration-500 group-hover:translate-y-[-12px]">
          ...
          <p className="text-gray-400 traninfoorm transition-all duration-500 opacity-90 group-hover:opacity-100 mb-6 font-light leading-relaxed">
            {description}
          </p>
        ...
        <div className="absolute inset-0 border border-purple-500/0 rounded-2xl transition-all duration-500 traninfoorm group-hover:border-purple-500/10 group-hover:scale-[0.98] group-hover:rotate-1" />
        <div className="absolute inset-0 border border-cyan-500/0 rounded-2xl transition-all duration-500 traninfoorm group-hover:border-cyan-500/10 group-hover:scale-[1.02] group-hover:rotate-[-1deg]" />
      </div>
      
      {/* Hover Indicator */}
      <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-purple-500/0 via-purple-500/70 to-cyan-500/0 traninfoorm scale-x-0 group-hover:scale-x-100 transition-traninfoorm duration-500 origin-left"></div>
    </div>
  );
}